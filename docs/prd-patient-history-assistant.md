...
### 11.2 동기 응답 지연 시간 실측 (Phase 5)

v1 PRD가 "단일 검색 도구 + ReAct 에이전트"를 전제로 동기 응답을 채택했는데, v2에서 그래프가 8노드로 커지면서 이 전제가 아직 유효한지 실측했다. `POST /patients/{patient_id}/chat`을 실제로 호출해 측정(재시도 루프가 없는 경우/있는 경우 모두 포함, top_k 기본값 5):

| 시나리오 | 환자(진단 건수) | 질문 | 재시도 루프 | HTTP 응답 시간 |
|---|---|---|---|---|
| brief | IP_0000013(4건) | "이 환자 최근 진료 이력 브리핑 해줘" | 없음 | 12.9초 |
| brief | IP_0000010(3건) | "환자 전체 진단 이력 브리핑 부탁해" | 없음 | 9.4초 |
| similar_cases | IP_0000001 | "이 환자와 비슷한 케이스 있어?" | 없음 | 15.7초 |
| similar_cases | IP_0000005 | "다른 환자들 중에 유사한 진단 사례 찾아줘" | 없음 | 14.3초 |
| answer | IP_0000007 | "이 환자 최근 진단 결과가 뭐였지?" | 없음 | 7.5초 |
| answer | IP_0000012 | "이 환자 두피 쪽 진단 받은 적 있어?" | 1회(grade_answer→generate) | 16.0초 |

정밀 벤치마크가 아니라 대략적인 실측치다(OpenAI API 호출 지연은 매 요청 변동이 있음). 그래프 노드 처리 시간만 떼어 직접 측정(FastAPI/DB 세션 오버헤드 제외)한 결과는 다음과 같다:

| 노드 | brief(재시도 없음) | similar_cases(재시도 없음) | answer(재시도 1회) |
|---|---|---|---|
| `route_query` | 1.9초 | 1.7초 | 3.2초 |
| `retrieve` | 0.7초 | 2.7초 | 0.7초 |
| `grade_documents` | 0.0초(결정적) | 1.7초(LLM) | 0.0초(결정적) |
| `context_organizer` | 0.0초 | 0.0초 | 0.0초 |
| `generate` (1차) | 5.2초 | 8.3초 | 2.0초 |
| `grade_answer` (1차, gpt-4o) | 3.0초 | 4.5초 | 1.7초 |
| `generate`/`grade_answer` (재시도) | — | — | +1.8초/+1.5초 |
| **그래프 처리 합계** | **10.9초** | **18.8초** | **10.8초** |

**15~20초 구간에 걸린 두 케이스의 원인**:
- **`similar_cases`(HTTP 15.7초, 그래프 처리만 18.8초)**: `generate`가 가장 큰 비용(8.3초, 전체의 약 44%) — 5개 사례 각각에 `shared_features`/`differences`/`clinical_note` 3개의 자유서술 필드를 채워야 해서 생성 토큰량이 `answer`/`brief`보다 훨씬 많다. `grade_answer`(gpt-4o, 4.5초)도 5건짜리 긴 구조화 응답 전체를 검증하느라 다른 시나리오보다 오래 걸린다.
- **`answer` + 재시도 1회(HTTP 16.0초)**: 이 케이스의 재시도는 실제로 정당했다 — 1차 답변이 "두피 진단 있음"이라고 썼지만 컨텍스트에는 `head_neck`(두피보다 넓은 부위)만 있어, `grade_answer`가 "두피로 단정할 근거 부족"으로 정확히 잡아내고 재생성을 요구했다(가드레일이 의도대로 동작한 사례이지 버그가 아님). 재시도 루프 1회는 `generate`+`grade_answer` 재실행으로 약 3.2초를 추가한다.
- **공통 요인**: `route_query`는 "검색 필요 여부"만 보는 가벼운 게이트인데도 매 턴 1.7~3.2초의 고정 LLM 왕복 비용이 든다 — v1(단일 ReAct 루프)보다 그래프의 필수 LLM 호출 횟수가 늘어난 근본 원인 중 하나(최소 경로도 route_query→retrieve→generate→grade_answer 4회 호출).

**결론**: 모든 케이스가 20초 미만이라 동기 응답 전제는 아직 유효하지만 여유가 크지 않다. `similar_cases`와 재시도가 발생하는 `answer`/`brief`는 10~20초대에 진입해 동기 HTTP 요청치고는 사용자 체감 대기가 길어지는 구간이다. 후속 개선 후보(이번 Phase 5 범위에는 없음, 기록만): `top_k` 기본값을 낮춰 `similar_cases`의 `generate` 부담을 줄이는 방안, 응답 스트리밍 전환, `route_query`를 경량 휴리스틱으로 대체하는 방안.

### 11.3 근거 인용(citations/similar_cases) 검증 (Phase 5)

`generate`가 만든 `answer` 타입의 `citations`와 `similar_cases` 타입의 `cases`가 실제 DB 원본과 정확히 일치하는지, 8개 (환자, 질문) 조합으로 검증했다 — 서로 다른 환자 8명, 질문 패턴도 구체적 질문/짧고 모호한 질문/전체 브리핑 요청을 섞었다.

| 환자 | 질문 | 응답 타입 | 검증 결과 |
|---|---|---|---|
| IP_0000001 | "이 환자와 비슷한 케이스 있어?" | similar_cases | 일치 |
| IP_0000005 | "다른 환자들 중에 유사한 진단 사례 찾아줘" | similar_cases | 일치 |
| IP_0000007 | "이 환자 최근 진단 결과가 뭐였지?" | answer | 일치 |
| IP_0000012 | "이 환자 두피 쪽 진단 받은 적 있어?" | answer | 일치 |
| IP_0000010 | "이 환자 하지 쪽 진단 이력 알려줘" | brief(에이전트가 브리핑으로 판단) | 일치 |
| IP_0000014 | "다른 환자 중에 비슷한 사례 있으면 알려줘" | similar_cases | 일치 |
| IP_0000003 | "이 환자 최근 진단 뭐였어?" (진단 1건뿐인 환자) | answer | 일치 |
| IP_0000019 | "유사한 케이스 찾아줘"(짧고 모호한 질문) | similar_cases | 일치 |

검증 방법: `answer` 타입은 `citations`의 `diagnosis_id`로 `diagnoses` 테이블을 직접 조회해 `date`/`anatomy_site`가 정확히 일치하는지, 그리고 그 `diagnosis_id`가 실제로 현재 세션 환자 소유인지 확인. `similar_cases` 타입은 `retrieve_patient_history` 도구가 반환한 원시 검색 결과(그 자체가 DB 쿼리 결과)와 `generate`가 만든 `cases`를 `anonymized_label` 기준으로 대조해 `date`/`anatomy_site`/`diagnosis`/`similarity`가 일치하는지 확인. 추가로 모든 응답 JSON을 문자열로 덤프해 현재 환자의 `patient_id`나 `IP_XXXXXXX` 형식의 식별자가 노출되는지 정규식으로 검사.

**결과**: 8건 전부 일치, 익명화 누출도 0건. Phase 4에서 이미 발견·수정한 3개 버그(§11.1)와 다른 새로운 할루시네이션 패턴은 이번 검증 범위에서는 발견되지 않았다. `IP_0000010`처럼 부위 관련 질문("하지 쪽 진단 이력")을 `generate`가 `answer`가 아니라 `brief`로 분류한 사례가 있었는데, 이는 3종 스키마 중 무엇을 고를지는 `generate`가 대화 의도에 따라 자유롭게 판단하도록 설계했기 때문(§9.3)이라 오류가 아니다 — `brief` 응답의 `timeline`도 원본 진단 이력과 정확히 일치함을 확인했다.
